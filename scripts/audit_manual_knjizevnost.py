...
manifest=json.load(open('redo/Knjizevnost_source_index.json',encoding='utf-8'))
seen={}; extras=[]; files=[]
for fn in sorted(glob.glob('redo/Knjizevnost*.json')):
    ...
    for pos,item in enumerate(data,1):
        idx=item.get('index')
        if not isinstance(idx,int) or idx<1 or idx>len(manifest):
            extras.append({'file':fn,'position':pos,'reason':'bad index','index':idx}); continue
        src=manifest[idx-1]
        if item.get('question') != src['question']:
            extras.append({'file':fn,'position':pos,'index':idx,'reason':'question not exact source match'}); continue
...
Path('redo/Knjizevnost_manual_audit.json').write_text(json.dumps(report,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
